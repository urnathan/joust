...
void
Option::Help (FILE *stream, char const *args) const
{
...
Option::Help (FILE *stream) const
{
  ...
  if (IsEnd ())
      ...
      if (HasParameter ())
	{
	  ...
	  len += fprintf (stream, "%c%.*s", " ="[IsConcatenated ()], l, colon);
	}
...
int
Option::Parse (int argc, char **argv, void *flags) const
{
  int argno = 1;
...
int
Option::ParseShort (char const *&arg, char const *next,
		    void *vars) const noexcept
{
  if (IsEnd ())
    return -1;
...
int
Option::ParseLong (char const *arg, char const *next, void *vars) const noexcept
{
  if (IsEnd ())
    return -1;
...
bool
Option::Parser<bool>::Fn (Flags, bool *var, char const *) noexcept
{
  if (!var)
    return false;
  *var = true;
...
bool
Option::ParseFalse (Flags, bool *var, char const *) noexcept
{
  if (!var)
    return false;
  *var = false;
...
bool
Option::Parser<signed>::Fn (Flags, signed *var, char const *param) noexcept
{
  if (var)
    {
      char *eptr;
...
bool
Option::Parser<unsigned>::Fn (Flags, unsigned *var, char const *param) noexcept
{
  if (var)
    {
      char *eptr;
...
bool
Option::Parser<char const *>::Fn (Flags, char const **var,
...
bool
Option::Parser<std::string>::Fn (Flags, std::string *var,
				 char const *param) noexcept
{
  if (var)
    *var = param;
...
bool
Option::Parser<std::string_view>::Fn (Flags, std::string_view *var,
				      char const *param) noexcept
{
  if (var)
    *var = param;
