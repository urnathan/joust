...
public:
  ...
  using ParseFn = bool (Flags flags, T *var, char const *param) noexcept;
  using HelpFn = void (FILE *, char const *pfx) noexcept;
...
public:
  ...
  {}
  constexpr Option (char const *longName_, char shortName_,
		    unsigned offset_, ParseFn<> parseFn_,
		    char const *helpText_, HelpFn *helpFn_ = nullptr) noexcept
...
#define OPTION_FN(FN) (::NMS::Option::Adaptor<FN>::Fn)
  ...
  constexpr Option (char const *longName_, char shortName_, Flags flags_,
		    ...
		    char const *helpText_, HelpFn *helpFn_ = nullptr) noexcept
    : Option (longName_, shortName_, flags_, offset_, Adaptor<Parser<M>::Fn>::Fn,
	      helpText_, helpFn_)
  {}
  template <typename C, typename M>
  constexpr Option (char const *longName_, char shortName_,
		    ...
		    char const *helpText_, HelpFn *helpFn_ = nullptr) noexcept
    : Option (longName_, shortName_, F_None, offset_, mptr_, helpText_, helpFn_)
  {}
  constexpr Option (Option const *subOptions_, void *subObject_) noexcept
    : shortName (1), subOptions (subOptions_), subObject (subObject_)
  {}
  constexpr Option (Option const *subOptions_, unsigned offset_) noexcept
    : shortName (1), offset (offset_), subOptions (subOptions_)
  {}

private:
  void *Var (void *f) const noexcept
  {
...
public:
  ...
  int Parse (int argc, char **argv, void *vars) const;

private:
  int ParseShort (char const *&opt, char const *next, void *vars) const noexcept;
  int ParseLong (char const *opt, char const *next, void *vars) const noexcept;
...
public:
  ...
  struct Adaptor
  {
    static bool Fn (Flags flags, void *var, char const *param) noexcept
    {
...
{
  static bool Fn (Flags flags, std::vector<T> *var, char const *param) noexcept
  {
    if (!var)
